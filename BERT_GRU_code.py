...
def universal_normalizer(df):
    df = df.copy()
    df.columns = [c.lower().strip() for c in df.columns]

    text_cols = [c for c in ["text","content","review","sentence"] if c in df.columns]
    label_col = next((c for c in ["label","sentiment","class"] if c in df.columns), None)
    ...
    def normalize(x):
        try:
            ...
            return 1 if str(x).lower() in ["positive","real"] else 0

    df["label"] = df[label_col].apply(normalize)
    return df[["text","label"]]

messages = pd.read_csv("/content/dataset_labels.csv")
messages = universal_normalizer(messages)
